...
const updateGoodies = async (id_goodies, goodiesData) => {
    const conn = await pool.connect();
    try {
        await conn.query('BEGIN');
        ...
        for (const taille of tailles) {
            const { id_taille, disponible } = taille;
            const quantite = disponible ? 1 : 0;

            await conn.query(`
                INSERT INTO Taille_goodies(id_goodies, id_taille, quantite_stock)
                VALUES ($1, $2, $3)
            `, [id_goodies, id_taille, quantite]);
        }

        await conn.query('COMMIT');
        return { success: true, message: "Goodies mises à jour avec succès" };
    } catch (error) {
